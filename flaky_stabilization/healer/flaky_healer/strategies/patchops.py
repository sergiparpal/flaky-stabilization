"""Serializable anchored patch operations and the apply/diff engine.

A ``PatchOp`` is a serializable anchored edit so recipes can store and replay it:
``{"file", "op": "replace"|"insert_before", "anchor", "old", "new"}``. ``anchor``
is a substring that locates the edit site; ``replace`` swaps the first occurrence
of ``old`` at-or-after the anchor, ``insert_before`` inserts ``new`` as a line
right before the line containing the anchor.

This module owns the *data type and the application machinery* only — it has no
dependency on the ``Strategy`` abstraction (strategies produce ``PatchOp``s; the
engine here applies them). Split out of ``base.py`` so a change to git-apply diff
generation no longer shares a file with the Strategy protocol and the registry.
"""
