...
root. That directory is ``flaky-stabilization`` (hyphenated, so not a
valid import name); the real implementation lives in the sibling
``flaky_stabilization`` package. This file just re-exports its
...
``flaky_stabilization`` importable by its real name in every case —
...
from flaky_stabilization import register  # noqa: E402  (after sys.path setup)
